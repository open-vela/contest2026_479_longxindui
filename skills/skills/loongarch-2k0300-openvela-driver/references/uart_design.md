...
LS2K0300 集成 4 路 16550 兼容 UART 控制器（UART0~UART3），用于串口字节收发。
每路 UART 有独立的波特率分频器、FIFO 与中断。Hummingbird 2K0300 板将 UART0
...
`ls2k0300_serial.c` 中注册。若内核驱动未为 UART2 注册实例，即使 defconfig
启用了 `CONFIG_LS2K0300_UART2`，`/dev/ttyS2` 也不会出现。需确认以下三处：

1. `nuttx/arch/loongarch/src/ls2k0300/Kconfig`：已定义 `LS2K0300_HAVE_UART2`
   与 `LS2K0300_UART2`、`UART2_BAUD/RXBUFSIZE/TXBUFSIZE`。
2. `nuttx/arch/loongarch/src/ls2k0300/ls2k0300_config.h`：`HAVE_UART_DEVICE`
   条件含 `|| defined(CONFIG_LS2K0300_UART2)`。
3. `nuttx/arch/loongarch/src/ls2k0300/ls2k0300_serial.c`：已定义
   `g_uart2priv`/`g_uart2port`（`.uartbase = LS2K0300_UART2_BASE`、
   `.irq = LS2K0300_IRQ_UART2`），并在 `loongarch_serialinit()` 中调用
   `uart_register("/dev/ttyS2", &g_uart2port)`。

> Hummingbird 2K0300 现有代码已补全以上三处，直接使用即可。
...
- `uart_ref_clk = LS2K0300_APB_FREQ * 1000000 = 200MHz`
...
`LS2K0300_UART2_BASE = 0x16100800`（非缓存）。
...
6. **波特率**：`LS2K0300_APB_FREQ = 200`（MHz），115200 对应分频值 109，
 ...
 * uart_template.c
 *
 * UART2 字节收发驱动模板 - 龙芯 2K0300 NuttX 平台
 *
 * Hummingbird 2K0300 板 UART2 引脚：
 *   UART2_TX → GPIO44 (MAIN_FUNC, 0x3)
 *   UART2_RX → GPIO45 (MAIN_FUNC, 0x3)
 *
 * 设备路径：/dev/ttyS2（由 ls2k0300_serial.c 注册）
 *
 * 编译条件：
 *   CONFIG_LS2K0300_UART2=y
