...
| B3 | `CONFIG_LS2K0300_PINCTRL=y` | defconfig 中存在（否则无 `/dev/pinctrl0`） |
...
| B8 | UART 内核 serial 驱动注册（UART 外设） | `ls2k0300_serial.c` 有 `g_uartNport` + `uart_register("/dev/ttySN", ...)`；`ls2k0300_config.h` 的 `HAVE_UART_DEVICE` 含目标 UART |
